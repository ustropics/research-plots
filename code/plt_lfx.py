# ---------------------------------------------------------------------------------------
# plt_center_flux_prl.py
#   Same logic as plt_center_prl.py but plots LH (latent heat flux) instead of wind speed
# ---------------------------------------------------------------------------------------
import numpy as np
...
# Set up logging
logging.basicConfig(
    filename='hurricane_ian_flux_processing.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)

# Function: Unstagger WRF variables (kept for completeness, though not used for LH)
...
# Function to compute annulus mean for a candidate center (unchanged)
...
if __name__ == '__main__':
    ...
    # <<< CHANGE >>> Define flux variable and colormap
    flux_var_name = 'LH'  # Latent Heat Flux
    cmap_flux = 'twilight'  # or 'viridis', 'plasma', etc.
    flux_levels = np.linspace(0, 1800, 21)  # Adjust based on your data range
    flux_label = 'Surface Latent Heat Flux (W/m²)'
    ...
    for t in range(start_timestep, num_timesteps):
        try:
            ...
            # ---------------------------------------------------------------------------------------
            # 3. Read U10/V10 for center finding (still needed!)
            # ---------------------------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------------------------
            # 5. Refine center using max tangential wind in RMW annulus
            # ---------------------------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------------------------
            # 6. Compute radial/tangential wind relative to refined center (still used for rings)
            # ---------------------------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------------------------
            # 7. <<< CHANGE >>> READ FLUX VARIABLE (LH)
            # ---------------------------------------------------------------------------------------
            # [south_north, west_east]
            flux = ds.variables[flux_var_name][t, :, :]
            flux = np.ma.filled(flux, np.nan)  # Ensure it's a regular array

            max_flux = np.nanmax(flux)
            min_pressure = np.min(psfc) / 100

            # ---------------------------------------------------------------------------------------
            # 8. Plotting with Cartopy
            # ---------------------------------------------------------------------------------------
            ...
            # <<< CHANGE >>> Plot flux instead of wind speed
            im = ax.contourf(lon, lat, flux, levels=flux_levels,
                             cmap=cmap_flux, transform=ccrs.PlateCarree())

            # Optional: Keep wind vectors for context (comment out if not wanted)
            ax.quiver(lon[::20, ::20], lat[::20, ::20], u10[::20, ::20],
                      v10[::20, ::20], scale=1500, transform=ccrs.PlateCarree())

            # Radial rings (same as original)
            ...
            fig.text(0.77, 0.91,
                     ...
                     f'Max LH: {max_flux:.1f} W/m² | Min P: {min_pressure:.1f} hPa',
                     ha='right', va='top', fontsize=10)
            fig.text(0.125, 0.89, time_formatted,
                     ha='left', va='top', fontsize=12)
            fig.text(0.77, 0.89, 'WRF Model - Surface Latent Heat Flux (W/m²)',
                     ha='right', va='top', fontsize=10)

            plt.subplots_adjust(top=1, bottom=0)

            output_dir = Path('figures_flux')
            output_dir.mkdir(parents=True, exist_ok=True)
            fig_path = output_dir / f'hurricane_ian_flux_{t}.png'
            ...
            # ---------------------------------------------------------------------------------------
            # 9. Log offset between pressure min and refined center
            # ---------------------------------------------------------------------------------------
