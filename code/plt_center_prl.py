...
# Set up logging
logging.basicConfig(
    filename='hurricane_ian_processing.log',
    level=logging.INFO,
...
# Function to compute annulus mean for a candidate center
...
def compute_annulus_mean(args):
    ...
    try:
        ...
        bins = np.linspace(0, search_radius_km, 50)  # Reduced bins for speed
...
if __name__ == '__main__':
    ...
    # Get lat/lon (static across timesteps)
    ...
    for t in range(start_timestep, num_timesteps):
        try:
            ...
            # ---------------------------------------------------------------------------------------
            # 4. Set initial best center to be minimum surface pressure
            # ---------------------------------------------------------------------------------------
            ...
            logging.info(
                f"Timestep {t} - Initial center (min surface pressure): {center_lat:.2f}°, {center_lon:.2f}°")
            print(
                f"Timestep {t} - Initial center (min surface pressure): {center_lat:.2f}°, {center_lon:.2f}°")
            ...
            candidates = [(i, j, lat, lon, u10, v10, search_radius_km,
                           annulus_width_km) for i in i_range for j in j_range]
            ...
            logging.info(
                f"Timestep {t} - Refined center (eyewall-based max tangential wind): {refined_lat:.2f}°, {refined_lon:.2f}°")
            print(
                f"Timestep {t} - Refined center (eyewall-based max tangential wind): {refined_lat:.2f}°, {refined_lon:.2f}°")
    ...
    print(
        f"Script completed. Total elapsed time: {t_end - t_start:.2f} seconds")
    ds.close()  # Close the NetCDF dataset
